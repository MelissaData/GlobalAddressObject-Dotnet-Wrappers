# Name:    BuildGlobalAddrNETdll
# Purpose: Compile the mdGlobalAddrNET.dll wrapper 
...
Write-Host "`n======================= WELCOME TO MELISSA GLOBAL ADDRESS OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdGlobalAddrNET.dll -platform:AnyCPU -target:library mdGlobalAddr_cSharpCode.cs" > $null

if (Test-Path ("mdGlobalAddrNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdGlobalAddrNET.dll"

  Write-Host "`nmdGlobalAddrNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdGlobalAddrNET.dll was not created successfully`n"
